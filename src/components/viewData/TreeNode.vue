    ...
    <ul class="tree">
      <li @click="parentNodeClick">
        <a href="javascript:void(0)" class="tree__link " :class='[{"tree__link--selected" : node.selected}, deloBgCorrection]'>
          ...
          <button v-if="isParent && isFolder" @click="toggle" class="tree__arrow" :class="{open: open}">
  ...
  computed: {
    ...
    dataNode() {
      ...
      switch (node.recType) {
        ...
        case constants.UCHASTUL:
        case constants.UCHASTOTHER: {
          let uchastBirthday = params.uchast_birthday
            ? `<p class="other-info">Дата рождения ${
                params.uchast_birthday
              }</p>`
            : "";
          let uchast_tip_name = params.uchast_tip_name
            ? `, ${params.uchast_tip_name.slice(0, -1).slice(1)}`
            : "";
          return `
              <h4><div>${
                params.uchast_vid_name_short
              }${uchast_tip_name}</div></h4>
        ...
        case constants.VEHSOTHER: {
          return `
              ...
              <p class="other-info">${params.vehs_marka_avto} ${
            params.vehs_modavto_name
          }</p>
            `;
        }
        case constants.VU_PRED:
