  ...
  export default {
    ...
    async created() {
      try {
        ...
        let vm = this;
        this.$store.watch(this.$store.getters.frmEdDecisRasmGetCommand, async () => {
          try {
    ...
    destroyed() {
      this.$store.dispatch('frmEdDecisRasmSetCid', null);
      this.$store.dispatch('frmEdDecisRasmSetData', null);
    },
    computed: {
      ...mapGetters({
        dataStore: 'frmEdDecisRasmGetData'
      }),
      body() {
        let res = null;
        if (this.dataStore) {
    ...
    methods: {
      async init() {
        try {
          let currentForm = innerFormStack.getCurrent();
          await this.$store.dispatch('frmEdDecisRasmSetCid', currentForm.cid);
